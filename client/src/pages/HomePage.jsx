...
function HomePage() {
  ...
  const { data: bookingData, loading: bookingLoading } = useQuery(GET_BOOKINGS, {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100 hover:shadow-2xl transition-all">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-3xl font-bold text-gray-800">Trek Details</h2>
              </div>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => selectedTrek && navigate(`/batch?trekId=${selectedTrek.id}`)}
            ...
            {data?.treks?.length === 0 ? (
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {data?.treks?.map((trek) => (
                  <div key={trek.id} className="border border-gray-200 rounded-xl overflow-hidden">
                    <div
                      onClick={() => setSelectedTrek(selectedTrek?.id === trek.id ? null : trek)}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="font-bold text-gray-800">{trek.name}</h3>
                    ...
                    {selectedTrek?.id === trek.id && (
                      <div className="p-4 bg-white border-t border-gray-200">
                        ...
                        {batchLoading ? (
                          ...
                          <p className="text-sm text-gray-500 text-center py-2">No batches available</p>
                        ) : (
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="flex-1">
                                    ...
                                    <div className="flex gap-3 mt-1 text-xs">
                                      <span className="text-gray-600">Capacity: {batch.capacity}</span>
                                      <span className="text-cyan-600 font-semibold">Booked: {batch.bookedSeats}</span>
                                    </div>
                                  </div>
                                  <span
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100 hover:shadow-2xl transition-all">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => selectedBatch && navigate(`/booking?batchId=${selectedBatch.id}`)}
            ...
            {!selectedBatch ? (
              <div className="text-center py-16 bg-gray-50 rounded-lg">
                ...
                <p className="text-gray-500 font-medium">Select a batch to view bookings</p>
                <p className="text-sm text-gray-400 mt-2">Choose a batch from the trek details</p>
              </div>
            ) : (
              <div>
                <div className="bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200 rounded-lg p-4 mb-4">
                  ...
                  <p className="font-bold text-gray-900">{new Date(selectedBatch.startDate).toLocaleDateString()}</p>
                  <div className="flex gap-4 mt-2 text-sm">
                    <span className="text-gray-600">Capacity: <span className="font-semibold">{selectedBatch.capacity}</span></span>
                    <span className="text-cyan-600">Booked: <span className="font-semibold">{selectedBatch.bookedSeats}</span></span>
                    <span className="text-green-600">Available: <span className="font-semibold">{selectedBatch.capacity - selectedBatch.bookedSeats}</span></span>
                  </div>
                </div>
                ...
                {bookingLoading ? (
                  ...
                  <div className="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <button
                      onClick={() => navigate(`/booking?batchId=${selectedBatch.id}`)}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-gray-900">{booking.travelerName}</p>
                              <p className="text-xs text-gray-600">ID: {booking.id.slice(0, 8)}...</p>
                            </div>
                          </div>
                          <span
